...
class RobotController(Node):
    ...
    def detect_stop_sign(self, pipeline, model):
        """Detects a stop sign in the camera feed with red color boosting."""
        ...
        # Red color boosting preprocessing
        hsv = cv2.cvtColor(color_image, cv2.COLOR_BGR2HSV)
        mask1 = cv2.inRange(hsv, (0, 70, 50), (10, 255, 255))
        mask2 = cv2.inRange(hsv, (160, 70, 50), (180, 255, 255))
        red_mask = mask1 | mask2
        red_boost = cv2.bitwise_and(color_image, color_image, mask=red_mask)
        enhanced_image = cv2.addWeighted(color_image, 0.7, red_boost, 0.3, 0)

        results = model.track(enhanced_image, verbose=False, persist=True, tracker="botsort.yaml")
            ...
            if result.boxes is not None:
                for box, cls_id, conf in zip(result.boxes.xyxy, result.boxes.cls, result.boxes.conf):
                    ...
                    if cls_name == "stop sign":
                        print(f"Detected stop sign with confidence {conf:.2f}")
...
def main(args=None):
    ...
    # Increased the resolution of the stream
